...
export class GraphComponent implements AfterViewInit, OnChanges {
  ...
  drawChildGraphNodes(
    ...
    if (parseInt(id.replace('child', ''), 10) <= 15) {
      ...
      if (seeIcons === undefined && !isStepChild && !this.isPreviewMode) {
      ...
      if (gNode == null || !gNode.isAddParent) {
        if (seeIcons === undefined && !this.isPreviewMode) {
          // hover on the down arrow, click on Edit Icon, height adjustment, placement of downarrow
          this.commonActionsOnAnyNode(
            mainChildGrp,
            downArrowImg,
  ...
  createStepChildRelationship(
    ...
    if(sourceGNode && destGnode){
      if (sourceGNode.id !== destGnode.id) {
        ...
        this.alertService.open(CONNECT_TO_SELF_WARNING_MESSAGE, AlertType.Warning);
      }
    }
  }
  ...
  commonActionsOnAnyNode(
    ...
    if (freeFormImg && freeFormImg !== null) {
  ...
  downArrowHoverActions(
    ...
    g.on('mouseover', function () {
      ...
      if (freeFormImg && showFreeFormIcon) {
  ...
  appendRect(
    ...
    if (!isChild && !(isAddParent && !this.isPreviewMode)) {
      g.append('rect')
        .attr('x', posX)
        .attr('y', posY)
        .attr('width', width)
  ...
  isStartingPointNode(currentNode: GraphNode) {
    return this.startingPointNodeIds.ids.includes(currentNode.id);
  }

  openDeleteDialog(parentId: number, childId: number): void {
    console.log('hrre->',parentId)
    console.log('hrre->',childId)
  ...
  delete(parentId: number, childId: number): void {
    ...
    this.store.dispatch(removeRelationship({ parentId: parentId, childId: childId}));
    //this.store.dispatch(createRelationship({ parentId: sourceGNode.id, childId: destGnode.id, isStep: true }));
  }
}
