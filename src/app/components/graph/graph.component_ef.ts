...
export class GraphComponent implements AfterViewInit, OnChanges {
  ...
  constructor(public renderer: Renderer2, private store: Store<State>) {}
  ...
  commonActionsOnAnyNode(
    ...
    console.log(isChild ? mainGrp.svgContainer : mainGrp.g);
  ...
  adjustHeight(foHeight: any, text: any): void {
    ...
    text._groups[0][0].parentNode.parentNode.parentNode.parentNode.setAttribute(
      'height',
      foHeight + 130  // this also changed by adding 20 =>VNS
    );
    text._groups[0][0].parentNode.parentNode.parentNode.parentNode.parentNode.style.height =
      foHeight + 117 + 'px'; // div needed for change - Fix for icon placement
  }
  ...
  appendLinks(
    ...
    if (parentId !== 0) {
      ...
      const line = svgContainer
        ...
        .attr('stroke-width', '1')
        .attr('opacity', 0.12)
        .attr("id","l_"+idgen)   // ID is not added =>VNS
        .attr('marker-end', 'url(#arrowhead)');

      if (id === 'child1') {
        line.attr('y1', lineY1); // added this to move the line for delete icon placement
