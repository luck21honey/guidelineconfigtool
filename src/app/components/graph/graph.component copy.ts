...
export class GraphComponent implements AfterViewInit, OnChanges {
  ...
  commonActionsOnAnyNode(
    ...
    this.adjustHeight(foHeight, textBox.text);
    downArrowImg.attr('y', foHeight + 85);
    if (freeFormImg !== null) {
      freeFormImg.attr('y', foHeight + 85);
    }
  }
  ...
  adjustHeight(foHeight: any, text: any): void {
    ...
    text._groups[0][0].parentNode.parentNode.parentNode.parentNode.parentNode.style.height =
      foHeight + 97 + 'px'; // div needed for change - Fix for icon placement
  }
  ...
  appendLinks(
    ...
    if (parentId !== 0) {
      ...
      if (id === 'child1') {
        line.attr('y1', lineY1 + 20); // added this to move the line for delete icon placement
