...
export class StartingPointComponent implements OnInit {
  ...
  showContent($event) {
    ...
    this.tooltip.style.cssText = `position: absolute; width: 300px; box-shadow: 0 3px 6px rgba(0,0,0,0.16), 0 3px 6px rgba(0,0,0,0.23); z-index: 1000; background: #fff; padding: 10px; overflow: hidden; height: 79px;`;
    this.tooltip.style.left = $event.pageX - 350 + 'px';
    this.tooltip.style.top = $event.pageY + 'px';
    this.tooltip.innerHTML = this.node.content;
